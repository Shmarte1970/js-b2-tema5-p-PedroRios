...
class InventoryManager {
    ...
    validateProduct(product) {
        ...
        // Validate types and format (code, discount, amount must be non-negative integers)
        if (
            ...
            throw new Error(`ERROR_DATA. Alguno de los datos del producto (${code}) no tiene un formato válido.`);
        }

        // Duplicate code
        if (this.#productList.some(p => p.code === code)) {
            throw new Error(`INVENTORY_CODE. Ya existe otro producto con ese código (${code}).`);
        }
        ...
        // Price >= 50
        if (price < 50) {
            throw new Error(`INVENTORY_PRICE. El precio del producto (${code}) no puede ser inferior a 50.`);
        }
